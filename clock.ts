class Clock {
  ...
  public toString() {
    ...
    if (this.hours < 0 && this.minutes < 0 ) {
      const negHours = 24 + (this.hours % 24) + Math.floor(this.minutes / 60)
      if (negHours > -24 ) {
      result.unshift(`${negHours}`) }
      if (negHours < -24) {
        const negNeg = (24 + negHours % 24)
        result.unshift(`${negNeg}`)
      }
      result.push((60 + (this.minutes % 60)).toString())
    }

    if (this.hours < 0 ) {
      const negHours = this.hours % 24
      if (result[0] === ':') {result.unshift(`${24 + negHours}`.length === 1 ? `0${24 + negHours}` : `${24 + negHours}`)}
      if (result[result.length - 1] === ':') {result.push(`${this.minutes}`.length === 1 ? `0${this.minutes}` : `${this.minutes}` )}
        return result.join('')
    }

    const hrs = this.hours % 24 + Math.floor(this.minutes / 60)
    if (this.minutes < -60) { const negMinHours = 24 + Math.floor(this.minutes / 60) + (this.hours % 24)
      if (negMinHours > -24 && result[0] === ':' ) {result.unshift(`${negMinHours}`.length === 1 ? `0${negMinHours}` : `${negMinHours}`)}
